...
public class JarFileIO {
    ...
    private void createJar() {
        ...
        try {
            FileChannel inputJar = new FileInputStream(in).getChannel();
            FileChannel outputJar = new FileOutputStream(out).getChannel();
            outputJar.transferFrom(inputJar, 0, inputJar.size());
            this.inputJar = new File(in);
            this.outputJar = new File(out);
            createDirInJar("instrumentor/examples");
        } catch (IOException e) {
    ...
    public void createDirInJar(String dir) {
        try {
            Path tmpFile = Paths.get(tempDir + "/" + dir);
            Files.createDirectories(tmpFile);
            Runtime.getRuntime().exec("jar uf " + outputJar + " -C " + tempDir + " " + tmpFile);
